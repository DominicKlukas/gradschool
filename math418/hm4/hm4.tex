...
\begin{enumerate}[leftmargin=1.2cm]
  ...
  Thus, the random variable $X(\omega) = \tan(\omega)$.
  ...
  We then compute:
  \begin{align*}
      F_X(z) = P(X(\omega) \le  z) &= P(\omega \le  \tan^{-1}(z))  \\
  ...
  \[
      f_X(z) = F_X'(z) = \frac{1}{\pi} \frac{1}{x^2 + 1}
  .\]
  ...
  By definition, 
  \[
  F_Y(x) = P(Y \le  x) = P(F(X) \le  x)
  .\] 
  Now, consider set $\{ \omega : F(X(\omega)) \le x \} $.
  Since $F$ is continuous, there exists some $y \in \R$ such that $F(y) = x$.
  Also, since $F$ is nondecreasing, and $\lim_{y \to \infty} F(y) = 1$, and $x < 1$ the set of $y$ such that $F(y) = x$ is bounded above.
  Now, let $y^{*} = \sup \{y \in \R : F(y) = x\}$.
  By continuity of $F$, we have that $F(y^{*}) = x$.
  In particular, if $X(\omega) > y^{*}$, then $F(X(\omega)) > x$, and since $F$ is nondecreasing we have that if $X(\omega) \le y^{*}$, then $F(X(\omega)) \le  x$.

  Therefore,
  \[
  \{\omega : F(X(\omega)) \le  x\}  = \{\omega : X(\omega) \le  y^{*}\} 
  .\] 
  However,
  \[
  P(X \le  y^{*}) = F(y^{*}) = x
  .\] 
  Therefore, $F_Y(x) = x$, so $Y$ has a uniform distribution.
  ...
  \textbf{Solution.}
  \begin{enumerate}
      \item We have that $\min \{n : X_n > X_0\} = n $ when $X_i \le  X_0$ where $1 \le  i \le  n-1$.
          This can then be described by the integral:
          \[
          P(N > n) = \int_{-\infty}^{\infty}f(x) F(x)^{n} dx
          ,\]
          where $f(x)$ is the pdf of $X_0$.
          We can immediately solve this with a substitution $u = F(x)$, since $F(-\infty) = 0, F(\infty) = 1$ and $\frac{du}{dx} = f(x)$ to get
          \[
          P(N > n) = \int_{0}^{1} u^{n} du = \frac{1}{n+1}
          .\] 
          Then, $F_N(n) = P(N \le  n) =  1 - \frac{1}{n+1}$.
      \item In order to compute this probability, we condition on the value of $N$.
          In particular, the conditional probability $P(X_N \le  x | N = n) = F^{n+1}$, since $X_0, \ldots, X_n \le  x$, and the ordering of these random variables sizes is already taken care of in $N = n$.
          Then, we compute:
          \begin{align*}
              P(X_N \le x) &= \sum_{n = 1}^{\infty} P(X_N \le  x | N = n)P(N = n)\\
              &= \sum_{n=1}^{\infty} F^{n+1}  \left( \frac{1}{n} - \frac{1}{n+1} \right)   \\
              &= F \sum_{n=1}^{\infty} \frac{F^{n}}{n} + F - \sum_{n=1}^{\infty} \frac{F^{n}}{n}\\
              &= F(- \log(1 - F)) + F + \log(1 - F) \\
              &= F + (1- F)\log(1-F)
          ,\end{align*}
          as desired.
