...
The lemma itself says that either we are in the cone, or there is some plane with a vector in the cone being the normal vector of that plane, and the plane is behind the cone, and the dot product with that vector is negative.
...
- Next, we consider the "reduced hessian"... the idea is, the update $p_{k}$ has the component in the range of $A_{k}$ updated such that the constraints are matched, and the component in the nullspace of $A_{k}$ is determined completely by the hessian, so by projecting onto the nullspace of $A_{k}$ we have the so-called "Reduced Hessian". Then, $p_{k}$ and $\lambda_{k}$ can be solved separately. Next, we split $p_{k}$ into the components in the range of $A_{k}$ and in the nullspace, and then argue that we can drop certain terms in the equations, depending on which ones are relevant for both the $\lambda$ and $p$ equations. (The cross terms of the quadratics, which deal with both parts of the split $p_{k}$.) After simplifying, we can use Hessian approximation methods in order to finish the job!
	- In the course of this reduced Hessian approach, we introduced a method to get an estimate for the Lagrange multipliers. $$
\hat{\lambda}_{k+1} = (A_{k}A^{T}_{k})^{-1} A_{k} \nabla f_{k}
$$
	- The idea behind this, is that the range of $A_{k}^{T}$ is precisely the space spanned by the constraints and so functions as $Y_{k}$ (see notation in the textbook.) By assuming that the component of $p$ that is parallel to the constraints is small, we can approximate with the component determined by the gradient of $\nabla f_{k}$.
...
- First, we find the smallest value that the constraints take, within a region smaller than the trust region: $\min_{v} \lvert \lvert A_{k}v + c_{k} \rvert \rvert^{2}_{2}$ subject to $\lvert  \lvert  v \rvert \rvert_{2} \leq 0.8 \Delta_{k}$, and set this minimum to $r_{k}$. Then, we solve the relaxation method problem: Quadratic SQP step, but subject to $A_{k}p + c_{k} = r_{k}$.
- The dogleg method is an attempt to improve the cauchy point. The cauchy point is the minimization (within the trust region) along the direction of local steepest descent. The problem is, this doesn't make full use of the second derivative. It only uses the second derivative to minimize the function in the specific direciton we are going.
- The dogleg method takes this a step further. First, go to the Cauchy point. Then, go towards the minimum of the quadratic problem. Going along these two line segments can be super helpful! Notation wise, we write $p^{U}$ as the Cauchy point and $p^{B}$ as the point that minimizes the quadratic.
- We can use this dogleg method for the minimization problem that finds $r$, even though $A_{k}$ is not a full rank matrix (and so it will be singular), we can find the solution which is closest.
- $\rho_{k}$ tells us how good our quadratic approximation of the merit function is.
#### S$\ell_{1}$QP (Sequential $\ell_{1}$ Quadratic Programming)
- We add the constraints on as a loss function with $\mu$ to be determined.
$$
